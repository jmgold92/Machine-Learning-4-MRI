...
%      end
%% Peform PCA
[loading,scores,~,~,expl]=pca(Xnormalized,'NumComponents',3);
figure(99);
subplot(131);  plot(cumsum(expl),'--o','LineWidth',2,'MarkerSize',22); xlabel('PC #'); ylabel('Cumulative Variance explained (%)'); xlim([0 5]);
subplot(132);  plot(xdata,loading(:,1:3),'--','LineWidth',2); ylabel('Magnitude'); xlabel('Offset');
legend('PC-1','PC-2','PC-3'); xlim([-5 20]); title('Eigen Vectors');
subplot(133);   bar(scores(1:2,:)); xlabel('Spectra #'); ylabel('Coefficient for each PC'); 
%% Plot data matrix
figure(); imshow(Xnormalized,[]); colormap('parula'); 

%% PLS on pH
Components=2:2:16;

for q=1:length(Components)
    [XL_pH,~,XS_pH,YL_pH,beta,pctvarpH] = plsregress(Xnormalized,Y,Components(q),'CV',63);
...
figure(99);
for q=1:16
    subplot(4,4,q); plot(xdata,XL_pH(:,q)); title(num2str(q));
end
%%
figure(98);
for q=1:16
    subplot(4,4,q); plot(YL_pH(:,q)); title(num2str(q));
end
